...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let iniMass = starWarsData[0].mass;
  let tempArr = arr.filter(obj => {
    if (parseInt(obj.mass) > iniMass) {
      return 1;
    } else 0;
  });
  let secArr = tempArr.map(val => {
    return val.name;
  });
...
const sortBy = (property, arr) => {
  // Solution code here...
  let i = property === 'name' ? Object.keys(arr[0])[0] : Object.keys(arr[0])[1]
  console.log(i);
  if (i === 'price') {
    return arr.sort((a, b) => {
      return a[`${i}`] - b[`${i}`];
    ...
    return arr.sort((a, b) => {
      let nameA = a[`${i}`].toLowerCase();
      let nameB = b[`${i}`].toLowerCase();
      let cond = (nameA < nameB) ? -1 : 1 || 0;
      return cond;
    });
  }
};
