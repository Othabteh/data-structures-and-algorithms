...
const createServer = () => {
  ...
  app.get("/hello", (req, res) => {
    res.status(200).send("hello form saif (back-end) to you front-end");
  });

  app.get("/aboutme", (req, res) => {
    res.status(200).send("I am a human, What do you want to know more?");
  });

  app.get("/favoritefoods", (req, res) => {
    const myFood = ["shawarma", "anything contain rice", "basically anything"];
    res.status(200).send(myFood);
  });

  app.get("/*", (req, res) => {
    res.status(404).send("don't know what are you trying to do");
  });
  // Solution code here...
...
const count = (target, input) => {
  ...
  let result = input.map((arr) => {
    let newArr = arr.reduce((comulative, num) => {
      if (num === target) {
        comulative++;
      }
      return comulative;
...
const totalSum = (input) => {
  ...
  let cutArrays = input
    .map((numbersArr) => {
      let sumOfEachArr = numbersArr.reduce((accumulative, number) => {
        return accumulative + number;
      }, 0);
      return sumOfEachArr;
    })
    .reduce((accumulative, number) => {
      return accumulative + number;
    }, 0);
  return cutArrays;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let resultArray = input.map((array) => {
    let remove = array
      .filter((number) => {
        if (number % 5 === 0) {
          return number;
